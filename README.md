...
The I/O part turns out to be surprisingly tricky, which is probably due to the fact that the sample image is a bit specific. Common libraries seem to handle 3-channel RGB .tiff (8-bit) fine, but they complain when trying to read 32-bit or 16-bit multiple channel or single channel .tiffs. 
...
```
Conclusion: I see a few questions on StackOverflow and OpenCV forum on the same issue, that never gets solved. It's possible that OpenCV's support for this kind of images is somewhat basic. 
...
**Thus moving to a hybrid approach : use Python to parse sample image and save as raw bytes. Read again in C++.**
